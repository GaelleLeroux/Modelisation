...
int main( int argc, char** argv )
{
  ...
  const auto jarvis = [&](std::vector<Point>& points)
  {
    trace.beginBlock("Jarvis");
    const int N = points.size();

    trace.info() << "Jarvis on " << N << " points." << std::endl;
    ...
    Point first = points[std::distance(points.cbegin(), min_it)];
    hull.push(first);
    ...
    do
    {
      ...
      for (int i = 1; i < N; i++)
      {
        if(polar_order(p, points[i]))
          p = points[i];
      }
      hull.push(p);
    }while(hull.top() != first);
    hull.pop();
    trace.endBlock();
  ...
  std::stack<Point> hull = jarvis( points );
