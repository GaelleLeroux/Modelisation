...
void myWidgetGL::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if(ctrl_pressed)
    {
        ...
        cpe::mat3 const mod = cpe::transposed(nav.camera_orientation());
        cpe::vec3 const t0 = cpe::vec3( x-nav.x_previous(),-y+nav.y_previous(),0.0f  );
        cpe::vec3 const t = 0.002f*mod*t0;

        scene_3d.translate_polygon_grid(t);
    }
...
myWidgetGL::myWidgetGL(const QGLFormat& format,QGLWidget *parent) :
    QGLWidget(format,parent),scene_3d(),nav(),draw_state(true)
{
...
void myWidgetGL::mousePressEvent(QMouseEvent *event)
{
    ...
    if(ctrl_pressed)
    {
        bool const addition_mode = (left_click==true);
        std::pair<cpe::vec3,cpe::vec3> ray=nav.ray_world_space_cam1(event->x(),event->y());
        scene_3d.picking_polygon_grid(ray.first,ray.second,addition_mode);
    }
...
void myWidgetGL::setup_camera()
{
    ...
    projection_matrix.set_projection_perspective(nav.field_of_view(),ratio,1e-4f,10.0f);
